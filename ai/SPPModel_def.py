...
def evaluate(data, model):
    ...
    mean_absolute_error = data["column_scaler"]["d1_diff_rate"].inverse_transform([[mae]])[0][0]
...
def predict(data, model, n_steps=100):
    ...
    # 스케일 된 값에서 실제 값으로 변환
    predicted_price = column_scaler["d1_diff_rate"].inverse_transform(prediction)[0][0]
...
def load_data(df, n_steps=100, lookup_step=1, test_size=0.2, shuffle=True):
    ...
    df['future'] = df['d1_diff_rate'].shift(-lookup_step)
...
def plot_graph(model, data):
    ...
    y_pred = np.squeeze(data["column_scaler"]["d1_diff_rate"].inverse_transform(y_pred))
