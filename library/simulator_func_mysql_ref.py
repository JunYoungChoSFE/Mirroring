...
# 볼린저밴드 알고리즘을 위해 추가된 부분 1
# simulator_func_mysql.py 파일 상단에 from pandas import DataFrame 아래에 추가해주세요
from library.trading_algorithms import *
...
class simulator_func_mysql:
    ...
    def variable_setting(self):
        ...
        # 아래 부터는 AI 알고리즘 별로 별도의 설정을 해주는 부분
            ...
            if self.simul_num == 22:
                ...
                # 시뮬레이팅 시작 일자
                self.simul_start_date = "20220301"
                # 매도 리스트 설정 알고리즘 번호
                self.sell_list_num = 21
                # AI알고리즘 사용 여부 (고급 챕터에서 소개)
                self.use_ai = True  # ai 알고리즘 사용 시 True 사용 안하면 False
                ...
                self.db_to_realtime_daily_buy_list_num = 6
                # 시뮬레이팅 시작 일자
                self.simul_start_date = "20220329"
                # 매도 리스트 설정 알고리즘 번호
                self.sell_list_num = 20
                # AI알고리즘 사용 여부 (고급 챕터에서 소개)
                self.use_ai = True  # ai 알고리즘 사용 시 True 사용 안하면 False
                ...
                self.db_to_realtime_daily_buy_list_num = 6
                # 시뮬레이팅 시작 일자
                self.simul_start_date = "20220201"
                # 매도 리스트 설정 알고리즘 번호
                self.sell_list_num = 21
                # AI알고리즘 사용 여부 (고급 챕터에서 소개)
                self.use_ai = True  # ai 알고리즘 사용 시 True 사용 안하면 False
        ...
        # ~!
        elif self.simul_num == 1001:
            # 시뮬레이팅 시작 일자
            self.simul_start_date = "20210406"

            ######### 알고리즘 선택 #############
            # 매수 리스트 설정 알고리즘 번호
            self.db_to_realtime_daily_buy_list_num = 1001
            # 매도 리스트 설정 알고리즘 번호
            self.sell_list_num = 21
            self.cut_time = 1200
            ###################################
    ...
    def db_to_realtime_daily_buy_list(self, date_rows_today, date_rows_yesterday, i):
        ...
        # 5 / 40 골든크로스 buy
            ...
            # 직전 종가 대비 상승한 종목 buy
            sql = "select * from `" + date_rows_yesterday + "` a where d1_diff_rate > 10 " \
                                                            "and NOT exists (select null from stock_konex b where a.code=b.code) " \
                                                            "and close < '%s' group by code"
            ...
            "and NOT exists (select null from stock_invest_danger g where a.code=g.code and g.post_date <= DATE('%s') and (g.cleared_date > DATE('%s') or g.cleared_date is null) group by g.code)"\
            "and a.close < '%s' " \
            "order by (d1_diff_rate * (volume - vol20)) desc"
            ...
            "order by (d1_diff_rate * (volume - vol20)) desc limit 5"
            ...
            "and a.close < '%s' " \
            "and NOT code_name = '넥슨지티' " \
            "order by (close * (volume - vol20)) desc"
        ...
        # ~!!
        elif self.db_to_realtime_daily_buy_list_num == 1001:
            # 20일 이동평균선과 승수 2 사용
            realtime_daily_buy_list = []
            # 사용하는 이동평균선 기간
            ma_period = 40
            ...
            if i > ma_period:
                ...
                sql = f"""
                        ...
                        WHERE NOT exists(SELECT null FROM stock_konex b WHERE a.code = b.code)
                            ...
                            ORDER BY volume * close DESC limit 500
                    """
        ...
        # 마지막 **db_to_realtime_daily_buy_list_num** if 조건문 제일 아래에 추가해주시기 바랍니다.
        # ~!!
        elif self.db_to_realtime_daily_buy_list_num == 1002:
            # 20일 이동평균선과 승수 2 사용
            realtime_daily_buy_list = []
            # 사용하는 이동평균선 기간
            ma_period = 40
        ...
        # 마지막 **db_to_realtime_daily_buy_list_num** if 조건문 제일 아래에 추가해주시기 바랍니다.
        # ~!!
        elif self.db_to_realtime_daily_buy_list_num == 1004:
            # 20일 이동평균선과 승수 2 사용
            realtime_daily_buy_list = []
            # 사용하는 이동평균선 기간
            ma_period = 40
        ...
        if len(realtime_daily_buy_list) > 0:
            ...
            if self.op != 'real':
                ...
                #!@@############
                # if self.use_ai:
                #    from ai_filter2_def import ai_filter
                #    ai_filter(self.ai_filter_num, engine=self.engine_simulator, until=date_rows_yesterday)

                if self.use_ai:
                    from ai_filter2 import ai_filter
                    ai_filter(self.ai_filter_num, engine=self.engine_simulator, until=date_rows_yesterday)

                # if self.use_ai:
                #     from ai_filter3 import ai_filter
                #     ai_filter(self.ai_filter_num, engine=self.engine_simulator, until=date_rows_yesterday)
    ...
    def db_to_all_item(self, min_date, df, index, code, code_name, purchase_price, yesterday_close):
        ...
        if df.loc[index, 'current_time'] is not None:
            self.df_all_item.loc[0, 'current_time'] = df.loc[index, 'current_time']
        if df.loc[index, 'AI_Pre'] is not None:
            self.df_all_item.loc[0, 'AI_Pre'] = df.loc[index, 'AI_Pre']
        if df.loc[index, 'AI_loss'] is not None:
            self.df_all_item.loc[0, 'AI_loss'] = df.loc[index, 'AI_loss']
        if df.loc[index, 'BB1'] is not None:
            self.df_all_item.loc[0, 'BB1'] = df.loc[index, 'BB1']
        if df.loc[index, 'BB2'] is not None:
            self.df_all_item.loc[0, 'BB2'] = df.loc[index, 'BB2']
        if df.loc[index, 'BB3'] is not None:
            self.df_all_item.loc[0, 'BB3'] = df.loc[index, 'BB3']
    ...
    def get_sell_list(self, i):
        ...
        elif self.sell_list_num == 22:
            # select 할 컬럼은 항상 코드명, 수익률, 매도할 종목의 현재가, 수익(손실)금액
            # sql 첫 번째 라인은 항상 고정
            sql = "SELECT code, rate, present_price,valuation_profit FROM all_item_db WHERE (sell_date = '%s') " \
                  "and (`current_time` > '%s' or present_price < yes_close * 0.9) group by code"
            sell_list = self.engine_simulator.execute(sql % (0, self.cut_time)).fetchall()

        elif self.sell_list_num == 23:
            # select 할 컬럼은 항상 코드명, 수익률, 매도할 종목의 현재가, 수익(손실)금액
            # sql 첫 번째 라인은 항상 고정
            sql = "SELECT code, rate, present_price,valuation_profit FROM all_item_db WHERE (sell_date = '%s') " \
                  "and (rate >= AI_Pre * '%s' or `current_time` > '%s') group by code"
            sell_list = self.engine_simulator.execute(sql % (0, self.AI_per, self.cut_time)).fetchall()

        elif self.sell_list_num == 24:
            # select 할 컬럼은 항상 코드명, 수익률, 매도할 종목의 현재가, 수익(손실)금액
            # sql 첫 번째 라인은 항상 고정
            sql = "SELECT code, rate, present_price,valuation_profit FROM all_item_db WHERE (sell_date = '%s') " \
        ...
        # 마지막 sell_list_num 확인 조건문 아래에 아래에 추가해주시기 바랍니다.
        # ~!!!
        elif self.sell_list_num == 1001:
            sell_list = []
            # 사용하는 이동평균선 기간
            ma_period = 40
            ...
            if i > ma_period:
                ...
                for item in sell_list_temp:
                    ...
                    bb_sql = f"""
                                    ...
                                    ORDER BY date DESC limit {ma_period}
                                """
                    df_close = self.engine_craw.execute(bb_sql).fetchall()
